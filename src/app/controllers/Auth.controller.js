...
class AuthController {
  ...
  async registerCustomer(req, res) {
    try {
      const { name, mail, email, password, phone } = req.body;
      const data = await AuthService.registerCustomer({ name, mail: mail || email, password, phone });
  ...
  async registerProvider(req, res) {
    try {
      const { name, mail, email, password, phone, display_name } = req.body;
      const data = await AuthService.registerProvider({ name, mail: mail || email, password, phone, display_name });
      return res.status(201).json(data);
    } catch (error) {
      return res.status(error.statusCode || 500).json({ message: error.message });
    }
  ...
  async providerLogin(req, res) {
    try {
      const { mail, email, password, token } = req.body;
      const response = await AuthService.providerLogin(mail || email, password, token);
      return res.status(200).json(response);
    } catch (error) {
      return res.status(error.statusCode || 500).json({ message: error.message });
    }
